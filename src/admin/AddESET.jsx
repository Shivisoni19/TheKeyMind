...
const AddESET = () => {
  const [ESETdata, setESETData] = useState(initialState);
  const {
    name,
    nprice,
    rprice,
  ...
  //   if (snapshot.exists()) {
  //     setESETData({ ...snapshot.setESETData() });
  //   }
  // };
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            setESETData((prev) => ({ ...prev, img: downloadURL }));
          });
        }
      );
    };
  ...
  const handleChange = (e) => {
    setESETData({ ...ESETdata, [e.target.name]: e.target.value });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSubmit(true);
    await addDoc(collection(db, "ESETdb"), {
      ...ESETdata,
    ...
    <>
      <div className="add-product container">
        {isSubmit ? (
          ...
          <>
            <h2 className="text-center mt30 mb20">Add ESET Products</h2>
