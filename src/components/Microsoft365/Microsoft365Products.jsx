...
const Microsoft365Products = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "office365db"), (snapshot) => {
        let list = [];
  ...
  const renderProducts = () => {
    return office365 && office365.map((item) => (
        ...
        <div className="container-fadeInTop">
          <div className="office-content">
            ...
            <div className="office-content-details fadeIn-top">
              ...
              <Button onClick={() => navigate(`/microsoftoffice365data/${item.id}`,{
                state:{microsoftoffice365Id: item.id}
              })} className="wishlist">
                 Go To Product
              </Button>
            </div>
