...
const AvastVPNProducts = () => {
  const [avastvpn, setAvastVpn] = useState([]);
  const [loading, setLoading] = useState([]);
  const navigate = useNavigate();

  useEffect(() => {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "avastvpndb"), (snapshot) => {
        let list = [];
  ...
  const renderProducts = () => {
    return avastvpn && avastvpn.map((item) => (
        ...
        <div className="container-fadeInTop">
          <div className="office-content">
            ...
            <div className="office-content-details fadeIn-top">
              ...
              <Button onClick={() => navigate(`/avastvpndata/${item.id}`,{
                state:{avastvpndataId: item.id}
              })} className="wishlist">
                 Go To Product  
              </Button>
            </div>
...
export default AvastVPNProducts;
