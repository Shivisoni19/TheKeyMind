...
const NortonProducts = () => {
  const [norton, setNorton] = useState([]);
  const [loading, setLoading] = useState([]);
  const navigate = useNavigate();

  useEffect(() => {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "nortondb"), (snapshot) => {
        let list = [];
        ...
        <div className="container-fadeInTop">
          <div className="office-content">
            ...
            <div className="office-content-details fadeIn-top">
              ...
              <Button onClick={() => navigate(`/nortondata/${item.id}`,{
                state:{nortondataId: item.id}
              })} className="wishlist">
                 Go To Product  
              </Button>
            </div>
