...
const MicrosoftOfc2019Products = () => {
  ...
  const [office2019, setOffice2019] = useState([]);
  const [loading, setLoading] = useState([]);
  const navigate = useNavigate();

  useEffect(() => {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "office2019db"), (snapshot) => {
        let list = [];
  ...
  const renderProducts = () => {
    return office2019 && office2019.map((item) => (
        ...
        <div className="container-fadeInTop">
          <div className="office-content">
            ...
            <div className="office-content-details fadeIn-top">
              ...
              <Button onClick={() => navigate(`/microsoftoffice2019data/${item.id}`,{
                state:{microsoftoffice2019Id: item.id}
              })} className="wishlist">
                 Go To Product
              </Button>
            </div>
