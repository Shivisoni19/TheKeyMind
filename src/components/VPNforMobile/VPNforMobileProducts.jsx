...
const VPNforMobileProducts = () => {
  const [vpnmobile, setVpnMobile] = useState([]);
  const [loading, setLoading] = useState([]);
  const navigate = useNavigate();

  useEffect(() => {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "vpnmobiledb"), (snapshot) => {
        let list = [];
  ...
  const renderProducts = () => {
    return vpnmobile && vpnmobile.map((item) => (
        ...
        <div className="container-fadeInTop">
          <div className="office-content">
            ...
            <div className="office-content-details fadeIn-top">
              ...
              <Button onClick={() => navigate(`/vpnmobiledata/${item.id}`,{
                state:{vpnmobiledataId: item.id}
              })} className="wishlist">
                 Go To Product  
              </Button>
            </div>
...
export default VPNforMobileProducts;
