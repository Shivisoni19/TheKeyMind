...
const HMA_VPNProducts = () => {
  const [hmavpn, setHmaVpn] = useState([]);
  const [loading, setLoading] = useState([]);
  const navigate = useNavigate();

  useEffect(() => {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "hmavpndb"), (snapshot) => {
        let list = [];
          ...
          <div className="office-content">
            ...
            <div className="office-content-details fadeIn-top">
             {/* <Link href="#" className="medium-button button-red add-cart">
              ...
              <Button onClick={() => navigate(`/hmavpndata/${item.id}`,{
                state:{hmavpndataId: item.id}
              })} className="wishlist">
                 Go To Product  
              </Button>
            </div>
